...
class Node(object):
    ''' This class stores the basic block instructions in terms of three
    types (as stated below in the constructor defination).'''

    def __init__(self, function_node, predicate_node, label_assignment_node):
        '''Initialize the class with a collection of:
        (1) "Function Node" that holds a basic block that has a single next address
        or no next address ("unknown");
        (2) "Predicate Node" that holds a basic block that has two next addresses,
        one for true and one for false; and
        (3) "Label Assignment Node" that holds an address to assign to the label.'''
        self._function_node = function_node
        self._predicate_node = predicate_node
        self._label_assignment_node = label_assignment_node

    def get_function_node(self):
        '''Return the collection of function node.'''
        return self._function_node

    def get_predicate_node(self):
        '''Return the collection of predicate node.'''
        return self._predicate_node

    def get_label_assignment_node(self):
        '''Return the collection of label assignment node.'''
        return self._label_assignment_node
...
def find_and_print(filename, explore=[]):
    ...
    with open(filename, "rb") as f:
        ...
        print("Entry Point (from ELF Header): %s" % hex(entry))
        ...
        # Check to see if it appears the C runtime is in use
        if(len(bbs) == 2):
            for val in bbs:
                if (sections.find(val).name != '.text'):
                    print("C runtime is in use because " + str(hex(val)) +
                    " address is in the " + str(sections.find(val).name) + " section")

                    # Figuring out where main is and adding it to the addresses to extract
                    address = find_main_function_address(entry, rad)
                    if(address):
                        explore = [address]
                        print("Disassemble the code from the new address")
                        # Make the pass one more time to
                        bbs = do_pass_one(explore, rad)

        node = do_pass_two(bbs, rad)    # Return the object
        print("\nLast basic block in the Function Node\n", node.get_function_node()[-1])
        print("Last basic block in the Predicate Node\n", node.get_predicate_node()[-1])
        print("Last address in the Label Assignment Node\n", node.get_label_assignment_node()[-1])

    print()

def find_main_function_address(entry, rad):
    ''' From the entry point derived from the ELF file, iterate
    through line by line. Find where the RDI mnemonic is printed
    and then compute the value that is going to reside inside RDI.
    That value is the address of the main function.'''
    ...
    while True:
        ...
        # Break when 'call' mnemonic is founnd as this is where
        # the code goes outside of the .text section
        if i.mnemonic == 'call':
            break

        i = rad.at(nextaddr)

    # 0x0000 mov rdi, 0x0000
    if(mnemonic == 'mov'):
        address = int(op_str.split(",")[-1].strip(), 16)
        print("Main function address: " + str(hex(address)))

    # 0x0000 lea rdi, [rip +/- 0x0000]
    elif(mnemonic == 'lea'):
        op_str = op_str.split(", ")[-1]
        op_str = op_str.replace('[', '')
        op_str = op_str.replace(']', '')
        if (op_str.split(" ")[0] == 'rip'):
            val = op_str.split(" ")[-1]
            arithmatic_operation = op_str.split(" ")[1]
            if(arithmatic_operation == "+"):
                address = next_addr + int(val, 16)
            else:
                address = next_addr - int(val, 16)
            print("Main function address: " + str(hex(address)))

    else: # Declare failure :(
        print("No computation is written for this instruction: %s" % op_str)
...
def do_pass_two(bbs, rad):
    ...
    '''Structure of the function and predicate node is the followiing: Nested List
    [   ["0x16248", "mov", "rdi, rbx"],
        ["0x1623d", "pop", "rbx"],
        [...],
        ...]

    Whereas the label assignment list holds the address for the starting of a block.
    This address will help us map the appearance of blocks.'''

    function_node_instructions = []
    predicate_node_instructions = []
    label_node_addresses = []
    ...
    for address in sort_bbs:
        ...
        label_node_addresses.append(hex(address))

        # This temp list will first store the entire block instructions, and
        # at the last based based on deciding function or predicate node it
        # will help the respective list store its value
        temp = []
        flag = 0    # 1 for function node and 2 for predicate node
        while address != None:
            # Disassemble the instruction.
            try:
                i = rad.at(address)
                temp.append([hex(address), i.mnemonic, i.op_str])
            except AddressException:
            ...
            nextaddr = i.address + i.size
            #print("print disassebly", address, i)
            ...
            if i.group(X86_GRP_CALL):
                if CALL_ENDS_BB:
                    ...
                    temp.append([hex(nextaddr), -1, -1]) # -1 indicates as flag
                    flag = 1    # Function node
                else:
                ...
                if i.mnemonic == 'jmp':
                    if is_imm(i.operands[0]):
                        ...
                        temp.append([i.op_str, -1, -1]) # -1 indicates as flag
                        flag = 1    # Function node
                    elif is_mem(i.operands[0]):
                        disp = is_rip_relative(i.operands[0])
                        if disp is not None:
                            print(f"next: {hex(nextaddr + disp)}")
                            temp.append([hex(nextaddr + disp), -1, -1]) # -1 indicates as flag
                            flag = 1    # Function node
                        else:
                            print("next: unknown")
                            temp.append(["unknown", -1, -1]) # -1 indicates as flag
                            flag = 1    # Function node
                    else:
                        print("next: unknown")
                        temp.append(["unknown", -1, -1]) # -1 indicates as flag
                    ...
                    temp.append([i.op_str, -1, "true"]) # -1 indicates as flag
                    temp.append([hex(nextaddr), -1, "false"]) # -1 indicates as flag
                    flag = 2    # Predicate node
        ...
        if(flag == 1):  # Function node
            if not function_node_instructions:  # Empty list
                function_node_instructions = temp
            else:
                function_node_instructions.append(temp)
        if(flag == 2):    # Predicate node
            if not function_node_instructions:  # Empty list
                predicate_node_instructions = temp
            else:
                predicate_node_instructions.append(temp)

    '''print(len(function_node_instructions), len(predicate_node_instructions), len(label_node_addresses))
    print(function_node_instructions[-1], predicate_node_instructions[-1], label_node_addresses[-1])'''

    # Package all basic blocks into Node instance.
    node = Node(function_node_instructions, predicate_node_instructions, label_node_addresses)
